...
export async function syncTicketStatus(ticketId: string): Promise<void> {
  ...
  try {
    const { status, statusCategory } = await fetchIssueStatus(ticket.key);
    db.prepare(
      "UPDATE Ticket SET jiraStatus = ?, jiraStatusCategory = ?, jiraCheckedAt = ?, jiraError = NULL WHERE id = ?"
    ).run(status, statusCategory, nowIso(), ticket.id);
  } catch (err) {
